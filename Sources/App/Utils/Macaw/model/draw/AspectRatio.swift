class AspectRatio {
    ...
    static let none: AspectRatio = AspectRatio()
    static let meet: AspectRatio = MeetAspectRatio()
    static let slice: AspectRatio = SliceAspectRatio()
    internal static let doNothing: AspectRatio = DoNothingAspectRatio()

    func fit(size: Size, into sizeToFitIn: Size) -> Size {
        return Size(w: sizeToFitIn.w, h: sizeToFitIn.h)
    }

    func fit(rect: Rect, into rectToFitIn: Rect) -> Size {
        return fit(size: rect.size(), into: rectToFitIn.size())
    }

    func fit(size: Size, into rectToFitIn: Rect) -> Size {
