...
struct MRectCorner: OptionSet {
    let rawValue: UInt

    static let none = MRectCorner(rawValue: 0)
    static let topLeft = MRectCorner(rawValue: 1 << 0)
    static let topRight = MRectCorner(rawValue: 1 << 1)
    static let bottomLeft = MRectCorner(rawValue: 1 << 2)
    static let bottomRight = MRectCorner(rawValue: 1 << 3)
    static var allCorners: MRectCorner {
...
extension MBezierPath {
    ...
    convenience init(roundedRect rect: NSRect, byRoundingCorners corners: MRectCorner, cornerRadii: NSSize) {
        self.init()
