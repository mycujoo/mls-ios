...
class BasicAnimation: Animation {
    ...
    override func delay(_ delay: Double) -> Animation {
        self.delay += delay
        return self
    }

    override func cycle(_ count: Double) -> Animation {
        self.repeatCount = count
        return self
    }

    override func easing(_ easing: Easing) -> Animation {
        self.easing = easing
        return self
    }

    override func autoreversed() -> Animation {
        self.autoreverses = true

        return self
    }

    override func cycle() -> Animation {
        self.cycled = true

        return self
    }

    override func onComplete(_ f: @escaping (() -> Void)) -> Animation {
        self.completion = f
        return self
    }
...
extension BasicAnimation: Hashable {
    ...
    static func == (lhs: BasicAnimation, rhs: BasicAnimation) -> Bool {
...
class AnimationDescription <T> {
    ...
    let valueFunc: (Double) -> T
    var duration = 0.0
    var delay = 0.0

    init(valueFunc: @escaping (Double) -> T, duration: Double = 1.0, delay: Double = 0.0) {
        self.valueFunc = valueFunc
        self.duration = duration
        self.delay = delay
    }

    func t(_ duration: Double, delay: Double = 0.0) -> AnimationDescription<T> {
