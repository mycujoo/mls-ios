...
class Shape: Node {
    ...
    let formVar: AnimatableVariable<Locus>
    var form: Locus {
        get { return formVar.value }
        set(val) { formVar.value = val }
    }

    let fillVar: AnimatableVariable<Fill?>
    var fill: Fill? {
        get { return fillVar.value }
        set(val) { fillVar.value = val }
    }

    let strokeVar: AnimatableVariable<Stroke?>
    var stroke: Stroke? {
        get { return strokeVar.value }
        set(val) { strokeVar.value = val }
    }

    init(form: Locus, fill: Fill? = nil, stroke: Stroke? = nil, place: Transform = Transform.identity, opaque: Bool = true, opacity: Double = 1, clip: Locus? = nil, mask: Node? = nil, effect: Effect? = nil, visible: Bool = true, tag: [String] = []) {
