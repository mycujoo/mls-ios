...
class Text: Node {
    ...
    let textVar: Variable<String>
    var text: String {
        get { return textVar.value }
        set(val) { textVar.value = val }
    }

    let fontVar: Variable<Font?>
    var font: Font? {
        get { return fontVar.value }
        set(val) { fontVar.value = val }
    }

    let fillVar: Variable<Fill?>
    var fill: Fill? {
        get { return fillVar.value }
        set(val) { fillVar.value = val }
    }

    let strokeVar: Variable<Stroke?>
    var stroke: Stroke? {
        get { return strokeVar.value }
        set(val) { strokeVar.value = val }
    }

    let alignVar: Variable<Align>
    var align: Align {
        get { return alignVar.value }
        set(val) { alignVar.value = val }
    }

    let baselineVar: Variable<Baseline>
    var baseline: Baseline {
        get { return baselineVar.value }
        set(val) { baselineVar.value = val }
    }

    let kerningVar: Variable<Float>
    var kerning: Float {
        get { return kerningVar.value }
        set(val) { kerningVar.value = val }
    }

    init(text: String, font: Font? = nil, fill: Fill? = Color.black, stroke: Stroke? = nil, align: Align = .min, baseline: Baseline = .top, kerning: Float = 0.0, place: Transform = Transform.identity, opaque: Bool = true, opacity: Double = 1, clip: Locus? = nil, mask: Node? = nil, effect: Effect? = nil, visible: Bool = true, tag: [String] = []) {
