...
class LinearGradient: Gradient {
    ...
    init(x1: Double = 0, y1: Double = 0, x2: Double = 0, y2: Double = 0, userSpace: Bool = false, stops: [Stop] = []) {
        self.x1 = x1
        self.y1 = y1
        self.x2 = x2
        self.y2 = y2
    ...
    convenience init(degree: Double = 0, from: Color, to: Color) {
        self.init(degree: degree, stops: [Stop(offset: 0, color: from), Stop(offset: 1, color: to)])
    }

    init(degree: Double = 0, stops: [Stop]) {
