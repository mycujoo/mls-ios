...
class ContentLayout {
    ...
    static let none: ContentLayout = NoneLayout()

    static func of(scaling: AspectRatio = .meet, xAlign: Align = .mid, yAlign: Align = .mid) -> ContentLayout {
        return ScalingContentLayout(scaling: scaling, xAlign: xAlign, yAlign: yAlign)
    }

    static func of(contentMode: MViewContentMode) -> ContentLayout {
        switch contentMode {
        case .scaleToFill:
    ...
    func layout(size: Size, into sizeToFitIn: Size) -> Transform {
        return layout(rect: size.rect(), into: sizeToFitIn)
    }

    func layout(rect: Rect, into sizeToFitIn: Size) -> Transform {
